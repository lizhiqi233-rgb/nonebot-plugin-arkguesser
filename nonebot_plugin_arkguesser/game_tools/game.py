...
class OperatorGuesser:
    def __init__(self):
        ...
        # 更新立绘路径到新的位置
        self.illustrations_path = Path(__file__).parent.parent / "resources" / "illustrations"
        
        # 初始化ZIP文件路径 - 必须在_load_data之前初始化
        self.illustrations_zip_path = Path(__file__).parent.parent / "resources" / "images" / "illustrations.zip"
    ...
    def start_new_game(self, uninfo: Uninfo) -> Dict:
        ...
        if not self.is_data_available():
            raise ValueError("数据文件不可用，请先使用 /arkstart 更新 数据库 来下载干员数据")
