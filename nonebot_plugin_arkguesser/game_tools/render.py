...
def get_zip_image_content(filename: str) -> Optional[bytes]:
    ...
    try:
        # 构建ZIP文件路径 - 更新到新的位置
        zip_path = Path(__file__).parent.parent / "resources" / "images" / "illustrations.zip"
...
def _new_illustration_to_data_uri(image_path: str) -> str:
    ...
    try:
        # 构建完整的文件路径
        full_path = Path(__file__).parent.parent / "resources" / "illustrations" / image_path
